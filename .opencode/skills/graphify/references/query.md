...
## ⚠️ Advertencia sobre `graphify query`

Basado en la evaluación en este proyecto: **`graphify query` es ruidoso**. Tiende a mezclar conceptos no relacionados en los resultados. **Siempre que sea posible, preferir:**

- `graphify explain "Concepto"` — para entender qué rodea a un componente (✅ recomendado)
- `graphify path "A" "B"` — para descubrir caminos entre dos conceptos (✅ recomendado)
- codebase-memory-mcp `search_graph(name_pattern="...")` — para encontrar algo específico

Usar `graphify query` solo cuando:
1. `explain` y `path` no aplican (pregunta muy abierta o exploratoria)
2. Se necesita una visión general de un tema amplio
3. Se quiere explorar qué comunidades/grupos tocan un tema

## ⚠️ Limitación de vocabulario

Graphify no entiende sinónimos ni stems. Si buscas en español y los nodos están etiquetados en inglés (o viceversa), el matcher literal devolverá 0 resultados. En ese caso, **usar codebase-memory-mcp** `search_graph(query="concepto")` que hace BM25 + búsqueda semántica.
