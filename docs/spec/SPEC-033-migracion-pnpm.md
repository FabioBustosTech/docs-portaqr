...
- **RF-3b (Negar telemetría).** Los scripts innecesarios (ej. `@scarf/scarf`, telemetría de un transitivo) se niegan con `false` **dentro del mismo mapa** — `ignoredBuiltDependencies` NO suprime el error en v11 (el install sigue fallando con `ERR_PNPM_IGNORED_BUILDS`, visto en backend 2026-09-04):
  ...
  allowBuilds:
    ...
    '@scarf/scarf': false
  ```
- **RF-3c (Cazar phantom deps).** El `tsc/jest` en host puede pasar con restos del `node_modules` de npm (pnpm mueve lo ajeno a `node_modules/.ignored` con WARN). La prueba que vale es el **build Docker limpio**: en backend expuso `import type { StringValue } from 'ms'` sin declarar (`ms@2.1.3` + `@types/ms@2.1.0` solo hoisteados por npm) → se declaran explícitos con paridad de versión del lock npm. Casos: `ms` (backend, rompía dev), `multer ^2.2.0` (backend, rompía PROD Railway con MODULE_NOT_FOUND en `node dist/main.js` — solo pega con `--prod`, fix 15d2963), `zod` + `@aws-sdk/client-s3` (cms, 500 en /admin). Regla: todo `Cannot find module` en Docker limpio = declarar la dep, no hoistear.
...
- **RF-5 (qr-cms: peers + patch MCP).** (a) Traducir `.npmrc` `legacy-peer-deps=true` → OJO pnpm 11 **solo lee auth/registry desde `.npmrc`**: poner `strictPeerDependencies: false` + `autoInstallPeers: true` en `pnpm-workspace.yaml` (resolver el conflicto `@modelcontextprotocol/sdk 1.26 vs 1.30` sin `--force`). (b) Reescribir `scripts/patch-mcp-zod4.mjs`: **prohibido escribir dentro de `node_modules`** (symlink al store read-only). Migrar a `pnpm patch @payloadcms/plugin-mcp` + `patchedDependencies` en `pnpm-workspace.yaml` (formato v11, no campo `pnpm` de package.json), manteniendo idempotencia y el mismo comportamiento (tools create/update exponen campos). Lecciones 2026-09-04 (qr-cms, commit a920350): el temp de `pnpm patch` es la raíz del paquete (script parametrizado con `MCP_PATCH_ROOT`/`MCP_PATCH_PKG`); `postinstall` eliminado; verificar con sufijo `_patch_hash` en `.pnpm` + marker en el store + symlink raíz apuntando a la copia parcheada; el Dockerfile debe `COPY patches/` en los 3 stages. Fantasmas cazados en Docker limpio (500 en /admin y /api): `zod ^3.25.76` (src/mcp-tools/uploadMedia.ts) y `@aws-sdk/client-s3 ^3.1114.0` (Media/hooks/delete-media-files.ts) — paridad con el lock npm.
...
- **RF-8 (docker-compose.yml).** `backend-portaqr.command: npm run dev` → `pnpm dev`; `qr-cms.command: npm run dev` → `pnpm dev`; `qr-app.command: sh -c "npm install || true && ... npm run build && npm start"` → `sh -c "pnpm install --frozen-lockfile || true && pnpm build && pnpm start"`. Mantener volúmenes anónimos `/app/node_modules` (siguen válidos con `.pnpm`). GOTCHAS 2026-09-04 (backend): (1) `docker compose run` **no rebuildea** — tras cambiar un Dockerfile hay que `compose build` primero o se levanta la imagen npm vieja; (2) `docker run --env-file` **no pela las comillas** de `JWT_PRIVATE_KEY="-----BEGIN...` (compose `env_file` sí) → la key llegaba con `"` literales y login daba 500 `secretOrPrivateKey must be an asymmetric key`. Probar SIEMPRE vía compose, no con `docker run` manual.
