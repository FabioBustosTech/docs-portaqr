...
- **RF-19**. **Página pública** (`https://portaqr.cl/qr/{idQr}` → `UrlList.tsx`): si `qrData.listImageUrl` existe, renderizar `<img>` responsiva entre el `nameData` y la lista de enlaces, centrada. Dimensiones: `max-w-full`, `max-h-32` (128px), `object-cover`, `rounded-lg`. No reemplaza el `nameData`.
