...
| 2026-08-12 | Equipo | **Validación E2E contra contenedores actualizados (59/59)** — 3 contratos FE↔BE rotos por forbidNonWhitelisted encontrados y corregidos (commit `0487651`, backend-portaqr): (1) **`vcardData` vs `vcard`**: el DTO `QrData` declaraba `vcard` pero el contrato real FE↔BD es `vcardData` (schema `qr.schema.ts` L132, el FE envía/lee `vcardData`) → los QRs vCard no se creaban; renombrado en DTO. (2) **`POST /scan/stats`**: el FE envía `scanDate`/`ip`/`referer` y `device.os`/`device.model` que ni DTO ni schema declaraban → los escaneos no se registraban; añadidos a DTO + schema + entidad + use-case + mapper (persistencia verificada en BD: `ip`, `referer`, `os`, `model`, `scanDate`). (3) **Throttler vs suite E2E**: los tests comparten IP y el límite de 5/min bloqueaba la suite (429) — `SENSITIVE_ENDPOINT_THROTTLE` ahora configurable por env (`THROTTLE_SENSITIVE_LIMIT`/`TTL`): 1000 en dev/CI, default 5/min en prod. Además: limpieza de mojibake (`vǭlido`→`válido`, `Ã­ndices`→`índices`) en los archivos tocados. Validación: tsc 0, 146 suites / 1139 tests, **E2E 59/59** (58 passed + 1 flaky en retry) |
