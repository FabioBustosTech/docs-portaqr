...
| 2026-09-03 | **Animación home**: la sección usa `<Reveal>` (mismo IntersectionObserver del resto del home, SPEC-024 S3). Solo home; blog/landing sin cambios. Commit `e8fb600`, verificado `reveal is-visible` en scroll. |
| 2026-09-03 | **Opción C + blog index**: en artículos el teaser es un RELACIONADO (misma categoría, sin fetch extra, nunca el propio); en `/blog` sección con el primero del listado. Helper `pickLatestPost(posts, excludeSlug)`. Commits qr-app `a3970eb`. Suite 577/577. |
| 2026-09-03 | **Sección en servicios+precios**: `NewsletterSection` con último post en `/servicios` (antes del footer, con Reveal) y `/precios` (entre FAQ y CTA ventas); nuevo origen `servicios` en CMS+frontend. Commits qr-app `51db4b0`, cms `bf4f76b`, e2e. Visual verificado en ambas. |
| 2026-09-03 | **Ajuste teaser real**: el destacado es el ÚLTIMO post (server fetch, excluye el actual en su página), clicable con miniatura (fallback marca) y URL relativa `/blog/<slug>` (local y prod sin vars). Helper `lib/newsletter-article.ts` + props `url/imageUrl` + cableado home/blog/landing. Commit `2c1c073`, suite 574/574, clic verificado en navegador hasta el artículo. |
| 2026-09-03 | **Ajuste fondos usuario**: superficies y textos a tokens del tema (`bg-card/border`, `text-foreground/muted`, inputs `bg-background/border-input`, foco `brand`); acentos mint solo en CTA pill, badge, checks y checkbox (familia del brand). Commit `e33991d`, suite 568/568, visual verificado en dark. |
| 2026-09-03 | **Ajuste v2b usuario**: consent SOBRE el botón; botón habilitado solo con email válido + nombre + consent (`canSubmit`); nombre mandatorio (antes opcional). Commits qr-app `671ecf9` + e2e (gating, nombre requerido, fix announcer). Suite 568/568, E2E newsletter 15/15. |
