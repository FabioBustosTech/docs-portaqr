...
- [ ] Estandarizar el tipo de `userId` entre colecciones: `qr.userId` es **string** (schema L89) y `pet-tag.userId` es **`Types.ObjectId`** — el `$facet` de H3 exige filtros con el tipo exacto por colección; unificar a un solo tipo eliminaría la fricción (requiere migración de datos + E2E de regresión).
- [ ] Considerar migrar la colección `pettags` (huérfana, 0 docs reales) → la colección efectiva es `pettagschemas` (Mongoose pluraliza `PetTagSchema`); limpiar `pettags` para evitar confusión en consultas ad-hoc.
...
| 2026-08-11 | Equipo | **Validación E2E + 3 bugs encontrados y corregidos** (rama `feat/spec-007-e2e` en e2e-tests-portaqr, 5 specs nuevos: H1 generate, H2 activación admin, H3 favorites-union, H4 dashboard-stats, H5 activate-pet-tag; +helpers BD). Contenedor docker reconstruido con la rama y validado: **suite E2E 59/59 verde** (12 nuevos + 47 preexistentes), unit 139/1043, tsc/eslint 0. Bugs: (1) **controller nunca llamaba `executeAdmin`** — la activación ADMIN creaba el registro sin activar los QRs (fix df9a851: `methodActivation === 'ADMIN'` → `executeAdmin`); (2) **`$limit` con string** — `page`/`limit` llegan como strings del query param y `$facet` exige número (`invalid argument to $limit stage`; fix 74876de: `Number(page)||1`/`Number(limit)||10`); (3) **`$match` sin cast por colección** — `qr.userId` es string (schema L89) y `pet-tag.userId` es `Types.ObjectId`; el `find()` de Mongoose casteaba el filtro pero el aggregate `$match` no → `total: 0` (fix 17f800b: filtro con tipo correcto por colección). Nota operativa: la colección real de pet-tags es `pettagschemas` (Mongoose pluraliza `PetTagSchema`), no `pettags` |
