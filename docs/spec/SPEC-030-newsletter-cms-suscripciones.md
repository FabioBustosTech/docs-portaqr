...
| 2026-09-04 | **Fix baja one-click Gmail + matriz re-suscripción verificada en vivo**: proxy `unsubscribe` acepta token en query con body no-JSON (RFC 8058, antes 400). Ciclo probado en local: pending→pending (1 doc, reenvía confirm), confirm→subscribed (token un solo uso + bienvenida), subscribed→subscribe (`already-subscribed` genérico, sin email extra), unsubscribe→re-alta (pending con tokens rotados, 1 doc). |
| 2026-09-04 | **Fix motivo de baja perdido + vista baja**: el motivo llegaba en 2do POST con usuario ya `unsubscribed` → early-return sin guardar. Ahora persiste si válido y no hay uno previo + nueva opción `prefer-not-to-say` (colección, tipos, front). Vista `/newsletter/baja` rediseñada (card + estados + botón mint + combo sin preset). Verificado en vivo. |
