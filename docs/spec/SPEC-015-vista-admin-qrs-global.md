...
| 2026-08-14 | **Ajuste v1.3 — unificar estado Inactivo (decisión del usuario)**: "inactivos y desactivados son básicamente un estado donde no están activos". Verificado en BD: ambos tienen `active:false`; el desactivado solo añade trazabilidad (`deactivatedAt`/`deactivatedBy`/`deactivationReason`, SPEC-014) y `expiration:null`. Cambios: (1) filtro `inactive` → `{ active: false }` (incluye vencidos + desactivados; conteo real 966+22=988); (2) `deactivated` se mantiene como filtro fino avanzado (`active:false` + `deactivatedAt` exists) para uso de API; (3) UI: filtro pasa a Todos/Activos/Inactivos (sin "Desactivados"); (4) badge Inactivo (amarillo) para todos los no activos, con tooltip "Desactivado por admin: <motivo>" cuando hay `deactivationReason`. Backend 1235 tests, frontend 102 tests verdes. Verificado en navegador: filtro Inactivos → 988 QRs, Página 1 de 99. |
| 2026-08-14 | **Ajuste v1.4 — filtro de usuario por pegado de ObjectId (decisión del usuario)**: el Select cargado con `userService.getUsers(1,100)` no escala (con miles de usuarios traerlos todos es ineficiente). Se reemplaza por un **input de pegado manual**: el admin copia el `userId` de la tabla (visible en monoespaciado + botón portapapeles por fila) y lo pega en el campo "Pegar ID de usuario (ObjectId)". Debounce 500ms: navega con `?userId=` solo si es un ObjectId de 24 hex válido (regex client-side, patrón `@IsMongoId` del backend); aviso "ID inválido (24 hex)" en caso contrario; botón X para limpiar. Sin llamada a getUsers al montar. Tests: 2 nuevos (pegado válido navega, inválido no navega + aviso), suite completa 104/104 verdes. Verificado en navegador: pegar `6a7e8a853c261d23492ee0c7` → 1 QR de ese usuario. |
| 2026-08-14 | **Ajuste v1.5 — salto directo a página (decisión del usuario)**: la paginación solo tenía Anterior/Siguiente; con 193 páginas era tedioso. Verificado que el backend ya acepta `page=N` arbitraria (solo frontend). Se añade al componente compartido `PaginationNav` (beneficia users, QRs, grid): input numérico "Ir a:" + botón Ir (o Enter), validación 1..totalPages (fuera de rango no navega), botón deshabilitado sin número, input se resetea tras saltar. Tests: 4 nuevos, suite completa 108/108 verdes, build OK. Verificado en navegador: saltar a página 50 → "Página 50 de 193" con los QRs correctos. |
| 2026-08-14 | **Bugfix — lookup del usuario dueño (user: null en todos los QRs)**: el usuario reportó "todos los usuarios dice usuario eliminado" aunque había usuarios en la BD. Diagnóstico: el pipeline del aggregate funcionaba correctamente en Mongo directo (`$convert` + `$lookup` + `$unwind` + `$facet` devolvía `userNames: ['fbustos']`), pero el código accedía a `doc.userInfo?.[0]` — tras el `$unwind` con `preserveNullAndEmptyArrays`, `userInfo` es el **OBJETO** del usuario (no un array), así que `[0]` devolvía `undefined` → `user: null` para todos. Los tests unitarios no lo detectaron porque el fixture simulaba `userInfo` como array. Fix: `const owner = doc.userInfo ?? null` + tipo `AdminQrAggregateDoc.userInfo` como objeto + fixture del test actualizado a la forma real de la BD (objeto). Tests: 1235/1235 verdes. Verificado: API devuelve 10/10 con user; tabla muestra nombre/email reales de los dueños. **Lección: al testear agregados con `$unwind`, el fixture debe reflejar la forma post-unwind (objeto), no pre-unwind (array).** |
| 2026-08-14 | **Ajuste v1.6 — tipo de QR legible en la tabla (decisión del usuario)**: la columna QR mostraba el tipo como valor crudo del enum (`dynamic`, `list`, `texto`...). Nuevo `src/lib/qr-type-labels.ts` con labels en español (Dinámico, Estático, WhatsApp, Email, Llamada, WiFi, Texto, Multilink, vCard, Pet Tag, Teléfono, Mapa) + badge azul por tipo en la columna QR; el filtro de tipo de la página reutiliza el mismo mapa (una sola fuente de verdad, `QR_TYPE_LABELS`). Tests: 4 nuevos, suite completa 112/112 verdes. Verificado en navegador: badges "Dinámico"/"Multilink"/"Texto" junto al id corto. |
| 2026-08-14 | **Ajuste v1.7 — multilink muestra los enlaces reales en Contenido (decisión del usuario)**: la columna Contenido de los QRs multilink mostraba "N enlaces" (ej. "1 enlaces"). Nueva `summarizeUrlList`: lista hasta 3 items con formato según typeUrl — URL sin protocolo (`ejemplo.cl/sitio`), `vCard: <fn>`, `PDF: <archivo>` — separados por coma, truncados por item (36/24 chars), con "+N más" cuando exceden. Tests: 3 actualizados, suite completa 114/114 verdes. Verificado en navegador: "ejemplo.cl/sitio", "ejemplo.cl/multilink", "ejemplo.cl/nombre". |
| 2026-08-14 | **Bugfix — búsqueda por tipo matcheaba todos los QRs del tipo (preexistente SPEC-008)**: los E2E nuevos revelaron que buscar "QR Multilink SPEC-015" devolvía 236 resultados sin relación (46 email + 89 whatsapp + 95 pet + 6 reales). Causa: las condiciones de tipo eran arrays `[{typeQr:'email'}, {campo:regex}]` que `.flat()` separaba en condiciones SUELTAS del `$or` → `{typeQr:'email'}` matcheaba TODOS los emails sin importar el término. Fix: cada condición de tipo es UN SOLO objeto `{ typeQr, campo/$or }` (AND implícito). Aplicado en `buildSearchConditions` (vista admin + findPaginatedByUser) y `findUserByFavorites` (mismo patrón). Test de regresión nuevo (typeQr nunca suelto). Backend 1236/1236 verdes. |
| 2026-08-14 | **E2E nuevos — vista global admin (6 tests, `tests/admin/qrs-admin-vista.spec.ts`)**: CA-05 (tabla con QRs/contador/paginación + acciones por fila — usa filtro de usuario para localizar QRs insertados por mongosh sin updatedAt, que el sort updatedAt:-1 manda al final), CA-02 (búsqueda por id y datos internos), CA-03 (filtros estado/tipo), v1.5 (salto de página), v1.4 (filtro usuario pegado + botón copiar), CA-07 (403 rol user). Suite chromium completa: **82/82 passed**. Nota: el repo e2e-tests-portaqr commitea en `main` (patrón de los specs previos SPEC-013/014). |
