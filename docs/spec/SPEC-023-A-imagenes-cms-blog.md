...
- **RF-7 (Naming según el post)**. El nombre del archivo se deriva del post al que pertenece la imagen:
  ...
  - Implementación: hook `afterChange` de `posts` (`syncMediaNaming`) recolecta `cover` + nodos `upload`/`imageLayout` del `content` (recursivo), y para cada media hace `payload.update` con `postSlug`, `usage` (`cover` | `inline`) y `filename` nuevo (Payload mueve el objeto en R2). El hook `beforeChange` de `media` usa `postSlug`/`usage` si vienen en el request (subida desde el contexto del post) para el nombre inicial.
...
> - **Renombrado en R2**: Payload 3 mueve el objeto en el adapter S3 al actualizar `filename` (copy + delete). Verificar en implementación; si el adapter no lo soporta, el hook re-subirá el buffer transformado con el nombre nuevo y eliminará el viejo.
> - **`data.file` en beforeChange**: en Payload 3 el archivo llega en `data.file.data` (Buffer) para uploads JSON y en `req.files.file` para multipart — el hook debe manejar ambos (o normalizar en el `beforeOperation`).
