...
export default function QrRedirect({ params }: QrRedirectProps) {
  ...
  const scanRegisteredRef = useRef(false);
  ...
  const registerScan = useCallback(async (locationData: LocationData): Promise<void> => {
    if (scanRegisteredRef.current) {
      console.log('Escaneo ya registrado, omitiendo registro duplicado');
      return;
    }
  ...
  const processQrRedirect = useCallback(async () => {
    ...
    try {
      ...
      // 3. Registrar el escaneo (una sola vez)
      await registerScan(locationData);
  ...
  }, [params.id, getGeolocation, registerScan, setProcessStatus, setErrorMessage, setIsLoading]);

  useEffect(() => {
    let mounted = true;

    if (mounted && !scanRegisteredRef.current) {
      processQrRedirect();
    }
  ...
  const handleRetry = async () => {
    ...
    setErrorMessage('');
    processingRef.current = false;
    // No reseteamos scanRegisteredRef para mantener el control de registros
